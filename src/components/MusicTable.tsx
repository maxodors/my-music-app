...
const renderValue = (col: string, value: any, row: RowData, index: number) => {
  ...
  if (col === "Название") {
    ...
    return url ? <a href={url} target="_blank" rel="noreferrer">{text}</a> : text;
  }
    ...
    return value
      ...
      .map((tag, i) => (
        <React.Fragment key={i}>
          <span className="tag" style={{ backgroundColor: getTagColor(tag) }}>{tag}</span>
          <br />
        </React.Fragment>
      ));
  }
  ...
  return <span className="tag" style={{ backgroundColor: getTagColor(text) }}>{text}</span>;
};
...
const MusicTable: React.FC<MusicTableProps> = ({ data }) => {
  ...
  return (
    <table className="music-table">
        ...
        <tr>
            {columnOrder.map(col => (
            <th key={col} className={`column-${col.replace(/\s/g, '-').replace(/[^a-zA-Z0-9-]/g, '')}`}>
