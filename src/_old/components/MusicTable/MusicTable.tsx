import { Anchor, Badge, Table, Text, rem } from '@mantine/core';
import React, { useState } from 'react';
import { columnOrder, columnTitles } from '../constants';
import { Filters, RowData } from '../types';
import { filterRows, getTagColor, isRowEmpty } from '../utils/musicUtils';
...
const MusicTable: React.FC<MusicTableProps> = ({ data }) => {
	...
	const visibleRows = () => filterRows(data, filters, columnOrder, isRowEmpty);
	...
	const renderValue = (
		...
		if (typeof value === 'string' && value.includes(',')) {
			return value
				.split(',')
				.map((tag: string, i: number) => renderTag(tag.trim(), i));
		}
